...
static char*
rpcsyncwerk_client_fret__string (char *data, size_t len, GError **error);

static int
rpcsyncwerk_client_fret__int (char *data, size_t len, GError **error);

static gint64
rpcsyncwerk_client_fret__int64 (char *data, size_t len, GError **error);

static GObject*
rpcsyncwerk_client_fret__object (GType gtype, char *data,
                            size_t len, GError **error);

static GList*
rpcsyncwerk_client_fret__objlist (GType gtype, char *data,
                             size_t len, GError **error);

static json_t *
rpcsyncwerk_client_fret__json (char *data, size_t len, GError **error);
...
RpcsyncwerkClient *
rpcsyncwerk_client_new ()
{
    return g_new0 (RpcsyncwerkClient, 1);
}

void
rpcsyncwerk_client_free (RpcsyncwerkClient *client)
{
    if (!client)
        return;
...
char *
rpcsyncwerk_client_transport_send (RpcsyncwerkClient *client,
                    ...
                    int n_params, ...)
{
    ...
    char *fret = rpcsyncwerk_client_transport_send (client, fstr, len, &ret_len);
    if (!fret) {
        g_free (fstr);
    ...
    if (strcmp(ret_type, "int") == 0)
        *((int *)ret_ptr) = rpcsyncwerk_client_fret__int (fret, ret_len, error);
    else if (strcmp(ret_type, "int64") == 0)
        *((gint64 *)ret_ptr) = rpcsyncwerk_client_fret__int64 (fret, ret_len, error);
    else if (strcmp(ret_type, "string") == 0)
        *((char **)ret_ptr) = rpcsyncwerk_client_fret__string (fret, len, error);
    else if (strcmp(ret_type, "object") == 0)
        *((GObject **)ret_ptr) = rpcsyncwerk_client_fret__object (gobject_type, fret,
                                                             ret_len, error);
    else if (strcmp(ret_type, "objlist") == 0)
        *((GList **)ret_ptr) = rpcsyncwerk_client_fret__objlist (gobject_type, fret,
                                                            ret_len, error);
    else if (strcmp(ret_type, "json") == 0)
        *((json_t **)ret_ptr) = rpcsyncwerk_client_fret__json(fret, ret_len, error);
    else
                         ...
                         GError **error, int n_params, ...)
{
    ...
    int ret = rpcsyncwerk_client_fret__int (fret, ret_len, error);
    g_free (fstr);
    g_free (fret);
    return ret;
}
                         ...
                         GError **error, int n_params, ...)
{
    ...
    gint64 ret = rpcsyncwerk_client_fret__int64 (fret, ret_len, error);
    g_free (fstr);
    g_free (fret);
    return ret;
}
                            ...
                            GError **error, int n_params, ...)
{
    ...
    char *ret = rpcsyncwerk_client_fret__string (fret, ret_len, error);
    g_free (fstr);
    g_free (fret);
    return ret;
}
                            ...
                            GError **error, int n_params, ...)
{
    ...
    GObject *ret = rpcsyncwerk_client_fret__object (object_type, fret, ret_len, error);
    g_free (fstr);
    g_free (fret);
    return ret;
}
                             ...
                             GError **error, int n_params, ...)
{
    ...
    GList *ret = rpcsyncwerk_client_fret__objlist (object_type, fret, ret_len, error);
    g_free (fstr);
    g_free (fret);
    return ret;
}
                          ...
                          GError **error, int n_params, ...)
{
    ...
    json_t *ret = rpcsyncwerk_client_fret__json (fret, ret_len, error);
    g_free (fstr);
    g_free (fret);
    return ret;
}
                                ...
                                void *vdata, const char *errstr)
{
    ...
    if (errstr) {
        ...
        if (strcmp(data->ret_type, "int") == 0) {
            ret = rpcsyncwerk_client_fret__int (retstr, len, &error);
            result = (void *)&ret;
        } if (strcmp(data->ret_type, "int64") == 0) {
            ret64 = rpcsyncwerk_client_fret__int64 (retstr, len, &error);
            result = (void *)&ret64;
        } else if (strcmp(data->ret_type, "string") == 0) {
            result = (void *)rpcsyncwerk_client_fret__string (retstr, len, &error);
        } else if (strcmp(data->ret_type, "object") == 0) {
            result = (void *)rpcsyncwerk_client_fret__object (
                data->gtype, retstr, len, &error);
        } else if (strcmp(data->ret_type, "objlist") == 0) {
            result = (void *)rpcsyncwerk_client_fret__objlist (
                data->gtype, retstr, len, &error);
        } else if (strcmp(data->ret_type, "json") == 0) {
            result = (void *)rpcsyncwerk_client_fret__json (retstr, len, &error);
        }
...
int
rpcsyncwerk_client_async_call_v (RpcsyncwerkClient *client,
                            const char *fname,
                            AsyncCallback callback,
                            const gchar *ret_type,
                            GType gtype,
                               ...
                               int n_params, ...)
{
    ...
    va_start (args, n_params);
    ret = rpcsyncwerk_client_async_call_v (client, fname, callback, "int", 0, cbdata,
                                      n_params, args);
    va_end (args);
    return ret;
}
                                 ...
                                 int n_params, ...)
{
    ...
    va_start (args, n_params);
    ret = rpcsyncwerk_client_async_call_v (client, fname, callback, "int64", 0, cbdata,
                                      n_params, args);
    va_end (args);
    return ret;
}
                                  ...
                                  int n_params, ...)
{
    ...
    va_start (args, n_params);
    ret = rpcsyncwerk_client_async_call_v (client, fname, callback, "string", 0, cbdata,
                                      n_params, args);
    va_end (args);
    return ret;
}
                                  ...
                                  int n_params, ...)
{
    ...
    va_start (args, n_params);
    ret = rpcsyncwerk_client_async_call_v (client, fname, callback, "object",
                                      object_type, cbdata,
                                      n_params, args);
    va_end (args);
    return ret;
                                   ...
                                   int n_params, ...)
{
    ...
    va_start (args, n_params);
    ret = rpcsyncwerk_client_async_call_v (client, fname, callback, "objlist",
                                      object_type, cbdata,
                                      n_params, args);
    va_end (args);
    return ret;   
                                ...
                                int n_params, ...)
{
    ...
    va_start (args, n_params);
    ret = rpcsyncwerk_client_async_call_v (client, fname, callback, "json",
                                      0, cbdata,
                                      n_params, args);
    va_end (args);
    return ret;
...
GObject*
rpcsyncwerk_client_fret__object (GType gtype, char *data, size_t len, GError **error)
{
    json_t  *object = NULL;
    GObject *ret = NULL;
    json_t  *member;
...
GList*
rpcsyncwerk_client_fret__objlist (GType gtype, char *data, size_t len, GError **error)
{
    json_t *object = NULL;
    GList  *ret = NULL;
