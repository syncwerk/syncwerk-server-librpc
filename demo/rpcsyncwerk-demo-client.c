...
#include "rpcsyncwerk-demo-packet.h"
...
/*The function is copied from rpcsyncwerk-server.c for convience*/
void
...
void
rpc_string_test(int sockfd, struct sockaddr_in *servaddr, RpcsyncwerkClient *rpc_client, GError *error)
{
    int ret;
    char str[16] = "hello rpcsyncwerk";
    /* call the client-side funcion */
    ret = rpcsyncwerk_client_call__int(rpc_client, "rpcsyncwerk_strlen", &error,
                                  1, "string", str);
    if (error != NULL) {
...
void
rpc_glist_test(int sockfd, struct sockaddr_in *servaddr, RpcsyncwerkClient *rpc_client, GError *error)
{
    int count = 4, len = 11;
    char str[16] = "A rpc test.";
    GList *ans=rpcsyncwerk_client_call__objlist(rpc_client, "rpcsyncwerk_objlisttest",
    ...
    rpcsyncwerk_set_objlist_to_ret_object (object,ans);
