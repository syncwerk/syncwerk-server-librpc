...
transport_read(TcpTransport *trans)
{
    ...
    rpcsyncwerk_client_generic_callback (pac->data, ret_len, trans->rpc_priv, NULL);
    trans->rpc_priv = NULL;
}
...
strlen_callback(void *vresult, void *user_data, GError *error)
{
    ...
    printf("the length of string 'hello rpcsyncwerk' is %d.\n", len);
}
...
main(int argc, char *argv[])
{
    ...
    rpcsyncwerk_client_async_call__int(rpc_client, "rpcsyncwerk_strlen",
                                  strlen_callback, "user data",
                                  1, "string", "hello rpcsyncwerk");
