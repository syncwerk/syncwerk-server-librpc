...
#if !defined(WIN32)
static const char *pipe_path = "/tmp/.rpcsyncwerk-test";
#else
static const char *pipe_path = "\\\\.\\pipe\\librpcsyncwerk-test";
#endif
...
/* sample client */
static RpcsyncwerkClient *client;
/* sample client with named pipe as transport */
static RpcsyncwerkClient *client_with_pipe_transport;
            ...
            size_t fcall_len, size_t *ret_len)
{
    ...
    ret = rpcsyncwerk_server_call_function ("test", temp, fcall_len, ret_len);
    g_free (temp);
    return ret;
}
                   ...
                   size_t fcall_len, void *rpc_priv)
{
    ...
    rpcsyncwerk_client_generic_callback (ret, ret_len, rpc_priv, NULL);
...
{
    RpcsyncwerkNamedPipeClient *pipe_client = rpcsyncwerk_create_named_pipe_client(pipe_path);
    cl_must_pass_(rpcsyncwerk_named_pipe_client_connect(pipe_client), "named pipe client failed to connect");
    return rpcsyncwerk_client_with_named_pipe_transport(pipe_client, "test");
}


void
test_rpcsyncwerk__simple_call (void)
{
    gchar* result;
    GError *error = NULL;
...
void
test_rpcsyncwerk__invalid_call (void)
{
    gchar* result;
    GError *error = NULL;

    result = rpcsyncwerk_client_call__string (client, "nonexist_func", &error,
                                         2, "string", "hello", "int", 2);
    cl_assert (error != NULL);
    g_free (result);
    g_error_free (error);
...
void
test_rpcsyncwerk__object_call (void)
{
    GObject *result;
    GError *error = NULL;

    result = rpcsyncwerk_client_call__object (client, "get_maman_bar",
...
void
test_rpcsyncwerk__objlist_call (void)
{
    GList *result, *ptr;
    GError *error = NULL;

    result = rpcsyncwerk_client_call__objlist (client, "get_maman_bar_list",
                                          MAMAN_TYPE_BAR, &error,
...
void
test_rpcsyncwerk__json_return_type (void)
{
    json_t *result;
    GError *error = NULL;

    result = rpcsyncwerk_client_call__json (client, "simple_json_rpc",
                                       &error, 2,
                                       "string", "year",
                                       "int", 2016);
    cl_assert (error == NULL);
...
void
test_rpcsyncwerk__json_param_type (void)
{
    json_t *result;
    GError *error = NULL;
    ...
    result = rpcsyncwerk_client_call__json (client, "count_json_kvs",
                                       &error, 1,
                                       "json", param);
...
void
test_rpcsyncwerk__simple_call_async (void)
{
    rpcsyncwerk_client_async_call__string (client, "get_substring",
...
void
test_rpcsyncwerk__simple_call_async_json (void)
{
    rpcsyncwerk_client_async_call__json (client, "simple_json_rpc",
                                    async_callback_json, NULL,
                                    2, "string", "hello", "int", 10);
}

void
test_rpcsyncwerk__pipe_simple_call (void)
{
    gchar* result;
    GError *error = NULL;

    result = rpcsyncwerk_client_call__string (client_with_pipe_transport, "get_substring", &error,
                                         2, "string", "hello", "int", 2);
    cl_assert_ (error == NULL, error ? error->message : "");
    cl_assert (strcmp(result, "he") == 0);
    g_free (result);
...
void
test_rpcsyncwerk__pipe_large_request (void)
{
    gchar* result;
    GError *error = NULL;
...
{
    RpcsyncwerkClient *client = do_create_client_with_pipe_transport();
    ...
    g_string_free (large_string, TRUE);
    rpcsyncwerk_free_client_with_pipe_transport(client);
...
// at the same time.
void
test_rpcsyncwerk__pipe_concurrent_clients (void)
{
...
void
test_rpcsyncwerk__initialize (void)
{
    rpcsyncwerk_server_init (register_marshals);
    rpcsyncwerk_create_service ("test");
    rpcsyncwerk_server_register_function ("test", get_substring, "get_substring",
                                     rpcsyncwerk_signature_string__string_int());
    rpcsyncwerk_server_register_function ("test", get_maman_bar, "get_maman_bar",
                                     rpcsyncwerk_signature_object__string());
    rpcsyncwerk_server_register_function ("test", get_maman_bar_list, "get_maman_bar_list",
                                     rpcsyncwerk_signature_objlist__string_int());
    rpcsyncwerk_server_register_function ("test", simple_json_rpc, "simple_json_rpc",
                                     rpcsyncwerk_signature_json__string_int());
    rpcsyncwerk_server_register_function ("test", count_json_kvs, "count_json_kvs",
                                     rpcsyncwerk_signature_json__json());

    /* sample client */
    client = rpcsyncwerk_client_new();
    ...
    RpcsyncwerkNamedPipeServer *pipe_server = rpcsyncwerk_create_named_pipe_server(pipe_path);
    cl_must_pass_(rpcsyncwerk_named_pipe_server_start(pipe_server), "named pipe server failed to start");
#if defined(WIN32)
...
void
test_rpcsyncwerk__cleanup (void)
{
    rpcsyncwerk_free_client_with_pipe_transport(client_with_pipe_transport);
